...
void gnuplot_plot_atmpfile_three (gnuplot_ctrl * handle, char const* tmp_filename, char const* title,
				char const* title2, char const* title3)
{
    char const *    cmd    = (handle->nplots > 0) ? "replot" : "plot";
    title                  = (title == NULL)      ? "(none)" : title;
    gnuplot_cmd(handle, "%s \"%s\" index 0 title \"%s\" with %s, \"%s\" index 1 title \"%s\" with %s, \"%s\" index 2 title \"%s\" with %s",
		cmd, tmp_filename, title, handle->pstyle, tmp_filename, title2, handle->pstyle, tmp_filename, title3, handle->pstyle) ;
    handle->nplots++ ;
    return ;
}
...
)
{
    ...
    if (handle==NULL || x==NULL || y==NULL || (n<1)|| x2==NULL || y2==NULL || (n2<1) || x3==NULL || y3==NULL || (n3<1)) return ;
    ...
    gnuplot_plot_atmpfile_three (handle,tmpfname,title,title2,title3);
}
...
)
{
  ...
  if (x==NULL || y==NULL || x2==NULL || y2==NULL || x3==NULL || y3==NULL || n<1 || n2<1 || n3<1)
	return ;
  ...
  gnuplot_plot_xy_three (handle, x, y, n, dataname.c_str(), x2, y2, n2, dataname2.c_str(), x3, y3, n3, dataname3.c_str());
